...
import { Plus, Home, X, Users, Info, Box } from "lucide-react";
  ...
  if (loading) return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="bg-card/40 backdrop-blur-sm p-8 rounded-[2.5rem] border border-white/5 shadow-lg">
            ...
            <div className="space-y-2">
              <div className="w-3/4 h-6 bg-muted/20 rounded-lg animate-pulse" />
              <div className="w-1/2 h-3 bg-muted/20 rounded-lg animate-pulse" />
            </div>
            <div className="mt-8 pt-6 border-t border-white/5 space-y-4">
              <div className="flex justify-between items-center">
                <div className="w-20 h-4 bg-muted/20 rounded-lg animate-pulse" />
                <div className="w-24 h-2 bg-muted/20 rounded-full animate-pulse" />
              </div>
              <div className="w-full h-10 bg-muted/20 rounded-xl animate-pulse" />
            </div>
          </div>
        ))}
      </div>
            ...
            return (
              <div key={room.id} className="bg-card/40 backdrop-blur-sm p-8 rounded-[2.5rem] border border-white/5 shadow-lg group hover:scale-[1.03] hover:shadow-2xl transition-all duration-300">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border ${!isFull ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'
                    }`}>
